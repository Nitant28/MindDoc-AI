...
chat_url = 'http://127.0.0.1:8001/api/chat/query'
...
try:
    ...
    if r.status_code == 200:
        ...
        r2 = requests.post(chat_url, json={'query': "What's the capital of France?"}, headers=headers, timeout=20)
        print('chat', r2.status_code, r2.text)
    else:
        print('Register failed; trying login to obtain token...')
        rlogin = requests.post('http://127.0.0.1:8001/api/auth/login', json=payload, timeout=10)
